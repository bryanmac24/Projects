...
@interface MultipleChoiceViewController ()<UITableViewDelegate, UITableViewDataSource,BEMCheckBoxDelegate,QLPreviewControllerDelegate,QLPreviewControllerDataSource>
...
- (void) didTapCheckBox :(BEMCheckBox *) checkBox{
    ...
    for (NSInteger index = 0; index  < self.checkList.count ; index ++)
    {
        if (index == row)
        {
            ...
            [newCheckList addObject:self.checkList[index]];
        }
    }
    ...
    for (NSInteger index = 0; index  < self.checkList.count ; index ++)
    {
        if ([self.checkList[index] isEqualToString:@"1"]){
            [self.answers addObject:self.survey.answerOptions[index]];
        }
    }
